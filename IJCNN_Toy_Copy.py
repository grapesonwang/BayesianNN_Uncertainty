...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 13 12:15:22 2020
...
def neural_network_regression(x, W_0, W_1, b_0, b_1):
...
ExpCov_0, ExpCov_1, ExpCbr, ExpExp  = uncertainty(x, sigmaW_0, sigmaW_1, sigmaB_0, sigmaB_1, meanW_0, meanW_1, meanB_0, meanB_1, Ksi, meanDelta)

#% Call the Uncertainty function after Variational Inference
VI_ExpCov_0, VI_ExpCov_1, VI_ExpCbr, VI_ExpExp = uncertainty(x, sigmaqW_0, sigmaqW_1, sigmaqB_0, sigmaqB_1, meanqW_0, meanqW_1, meanqB_0, meanqB_1, Ksi, meanqDelta)
...
#loss_x_value = tf.reshape(VI_ExpCov_0[:,0], [tf.size(VI_ExpCov_0[:,0]), 1])
...
#loss_y_predict = 2 * loss_y_predict * (0.5 + opt)

#loss_y_predict = 2 * opt * loss_y_predict + loss_y_predict
...
#loss_b = tf.constant(tf.zeros([50, 1]))
...
inference = ed.KLqp({W_0: qW_0, b_0: qb_0,
                     W_1: qW_1, b_1: qb_1, Delta: qDelta}, data={y: y_train})
...
#loss_x_value = tf.reshape(otpt_VI_ExpCov_1[:,0], [tf.size(otpt_VI_ExpCov_1[:,0]), 1])
...
for _, i in zip(range(inference.n_iter), range(inf_iter)):
  ...
  for step in range(10):
    ...
    print(step)
    print(sess.run(loss_w)) 
    
  #% Calibrated data
  loss_w = sess.run(loss_w)
  loss_opt = 1.0/loss_2 - 1.0
  new_CbrWeight = 1.0/(2.0*(1.0 + loss_opt))
  new_mnLyr_1_node_1[:, i] = 2.0 * otpt_VI_ExpCov_1[:, 0] * new_CbrWeight
  new_sigmaLyr_1_node_1[:, i] = otpt_VI_ExpCov_1[:, 1]  
